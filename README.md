...
This project explores using immersive 3D visualization to explore and debug electric grid topology.

## Why I Chose It:

I wanted to build something connected to real-world grid operations while also exploring a creative interface. Grid topology is inherently spatial, so using VR enables more intuitive reasoning and interaction.
...
- Chose A-Frame for easy WebXR support and rapid VR development.
...
- UI: extended dynamic styling and VR headset compatibility.
...
- **3D Scene Rendering**: Uses A-Frame + 3D-force-graph to create the interactive scene.
- **UI Filters**: Allow dynamic filtering by voltage, owner, region.
- **Interaction**: Hovering on a node or edge reveals its metadata in an info panel.

This project is a demonstration of how immersive tools can empower engineers working with large-scale infrastructure data.
